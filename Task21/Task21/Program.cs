...
namespace Task21
{
    internal class Program
    {
        ...
        {
            MyTreeMap<string, int> tree = new MyTreeMap<string, int>();
            tree.Put("A", 1);
            tree.Put("C", 3);
            tree.Put("B", 2);
            tree.Put("J", 5);
            ...
            Console.WriteLine(tree.PollLastEntry());
            tree.Print();
        }
    }
    class MyTreeMap<K, V> where K : IComparable<K>
    {
        ...
        private void RecursionPrint(Node<Pair<K, V>> p)
        {
            if (p != null)
            {
                RecursionPrint(p.left);
                Console.Write("(" + p.value.key + ": " +
                    p.value.value + ") ");
